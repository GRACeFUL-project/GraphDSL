...
declareVars ((z, ms,  outs, ins):rs) =
  ...
  : declareVars rs ++
    [Declare (Dec, Int) (propIdent z n) Nothing
    | (n, ls) <- outs ++ ins]
...
constraint1a1 z outs ins     =
  [regularComb $ ArrayLit ([Var $ propIdent y z | (y,s2) <- delete (x, s1) (ins ++ outs)] ++
                           [IConst $ signToInt s1, Var $ propIdent z x]) | (x, s1) <- outs]

constraint1a2 :: Node -> [(Node, Sign)] -> [(Node, Sign)] -> [Item]
constraint1a2 z [] ins   = [regularComb $ ArrayLit [IConst $ signToInt Z, IConst $ signToInt s1,
                                                    Var $ propIdent z x]
                           | (x, s1) <- ins]
constraint1a2 z outs ins = [regularComb $ ArrayLit ([Var $ propIdent y z | (y,s2) <- outs] ++
                                                    [IConst $ signToInt s1, Var $ propIdent z x])
                           | (x, s1) <- ins]

constraint1b :: Node -> [(Node, Sign)] -> [(Node, Sign)] -> [Item]
constraint1b z outs ins = [regularPlus $ ArrayLit ([Var $ propIdent x z | (x,s) <- outs ++ ins] ++
                                                   [Var $ varIdent z]) ]
...
-- iSolveCLD :: DSL.GraphDSL.GraphSyntax a -> IO Interfaces.FZSolutionParser.Solutions
iSolveCLD :: GraphSyntax a -> IO Solutions
